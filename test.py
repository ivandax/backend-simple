...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_patch_project_success(self):
        res = self.client().patch("/projects/1", json=self.projectPatchPayload, headers={"Authorization": self.validToken})
        data =json.loads(res.data)
        self.assertEqual(res.status_code, 200)
    ...
    def test_patch_task_success(self):
        res = self.client().patch("/tasks/1", json=self.taskPatchPayload, headers={"Authorization": self.validToken})
        data =json.loads(res.data)
        self.assertEqual(res.status_code, 200)
